...
impl Instance {
    ...
    pub fn load_from_hgr(mut reader: impl BufRead) -> Result<Self> {
        ...
        // First pass: find problem line and count edges
        while reader.read_line(&mut line)? > 0 {
            ...
            if line.starts_with('p') {
                ...
                ensure!(parts.len() == 4, "Invalid problem line format");
                ensure!(parts[1] == "hs", "Expected 'hs' problem descriptor");
                num_nodes = parts[2].parse()?;
        ...
        ensure!(num_nodes > 0, "Number of nodes must be positive");
        ensure!(num_edges > 0, "Number of edges must be positive");
        ...
        let instance = Self::load(num_nodes, num_edges, |handler| {
            while reader.read_line(&mut line)? > 0 {
                ...
                if trimmed_line.is_empty() || trimmed_line.starts_with('c') {
                    ...
                    // Parse edge vertices (subtract 1 from each vertex number since they start at 1)
                    let vertices: Result<Vec<usize>> = line
                        .split_whitespace()
                        .map(|s| {
                            let v: usize = s.parse()?;
                            ensure!(v > 0 && v <= num_nodes, "Vertex number out of range: {}", v);
                            Ok(v - 1) // Convert to 0-based indexing
                        })
                        .collect();
                    handler.handle_edge(vertices?.into_iter().map(Ok))?;
                line.clear();
            }
            Ok(())
        })?;
